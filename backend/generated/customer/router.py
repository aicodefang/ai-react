...
router = APIRouter(prefix="/customers", tags=["generated-customer"])
...
async def create_customer(payload: SaveCustomerRequest):
    data = await customers.create_customer(get_settings(), payload)
    return success(data, message="客户创建成功")


@router.put("/{customer_id}")
async def update_customer(customer_id: str, payload: SaveCustomerRequest):
    data = await customers.update_customer(get_settings(), customer_id, payload)
    return success(data, message="客户更新成功")
